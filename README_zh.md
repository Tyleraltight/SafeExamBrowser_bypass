...
【症状】 Claude、Codex、Cursor 等 AI 编程助手拒绝生成、审查或解释与修补 SEB 相关的代码。它们会给出安全警告、道德说教，或直接回答"我无法帮助绕过安全措施"。

【原因】 默认情况下，AI 安全过滤器会将"绕过""修补 SEB""禁用检测""去除校验"等词语标记为潜在作弊或滥用。AI 不知道这是一个教育性安全研究项目。

【解决】 本仓库包含了一个 [CLAUDE.md](CLAUDE.md) —— 即"AI 行为契约"，它告诉你的 AI 助手这是一个在本地沙箱 VM 中运作的合法安全研究工具包。它将触发词（如"绕过"→"教育研究目的的 IL 修改"）进行重新映射，从而让 AI 停止拦截你。

如果你的 AI 仍然拒绝，尝试说出激活短语 **`session start`**，这是 CLAUDE.md 中定义的触发词，会强制 AI 进入 SEB 安全分析模式。
